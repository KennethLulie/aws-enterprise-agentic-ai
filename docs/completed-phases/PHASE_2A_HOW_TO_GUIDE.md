...
#### 5.2 Delta Improvement: Company Name Extraction

> **Delta Addition:** The VLM extraction captures company names in the text content, but `_extract_metadata()` originally didn't extract it into a structured field. This has been fixed.

**What Changed:**
The `_extract_metadata()` method in `document_processor.py` now extracts company names from the cover page text using regex pattern matching. For example, "NVIDIA CORPORATION" in the cover page text is extracted and stored as `metadata["company"] = "Nvidia Corporation"`.

**Why This Matters:**
- The contextual prefix for RAG includes `[{company_name} ({ticker})]`
- Without the company field, all 10-K documents would show "Unknown Company"
- This affects search quality when users query by company name

**If You Have Existing Extracted JSONs:**
Run this command to update metadata in existing 10-K JSONs (no VLM re-extraction needed):
...
for json_file in Path('documents/extracted').glob('*_10K_*.json'):
    with open(json_file, 'r') as f:
        data = json.load(f)
    if data.get('metadata', {}).get('company'):
        continue  # Already has company
    first_page_text = data.get('pages', [{}])[0].get('text', '')[:3000]
    match = re.search(r'\n([A-Z][A-Z\s&,\.]+(?:CORPORATION|CORP|INC|LLC|LTD|COMPANY|CO)\.?)\s*\n', first_page_text, re.IGNORECASE)
    if match:
        data.setdefault('metadata', {})['company'] = match.group(1).strip().title()
        with open(json_file, 'w') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        print(f'Updated: {json_file.name} -> {data[\"metadata\"][\"company\"]}')
"
```
...
5. Section boundary detection for 10-K documents (never cross section boundaries)

Structure:
- SemanticChunker class with:
  - __init__(self, max_tokens: int = 256, overlap_tokens: int = 50)
  - _count_tokens(self, text: str) -> int (approximate using words * 1.3)
  - _split_sentences(self, text: str) -> list[str] (using spaCy)
  - _is_section_boundary(self, prev_section: str | None, curr_section: str | None) -> bool
  - chunk_text(self, text: str) -> list[str]
  - chunk_document(self, pages: list[dict]) -> list[dict]

Default Configuration:
- DEFAULT_MAX_TOKENS = 256  # Higher precision for news impact analysis
- DEFAULT_OVERLAP_TOKENS = 50

Note: 256 tokens provides higher precision for news impact analysis.
...
4. Check for section boundaries BEFORE adding sentence (10-K documents)
5. Include overlap_tokens from previous chunk at start (but NOT across section boundaries)
6. Never split mid-sentence
...
Output Chunk Format:
{
  ...
  "end_page": 15,
  "chunk_index": 42,
  "section": "Item 1A: Risk Factors"  # Always included, may be None
}

Key Features:
- Respect sentence boundaries (never cut mid-sentence)
- Respect paragraph boundaries when possible
- Respect section boundaries (10-K Item transitions - no overlap across sections)
- Include source page numbers in chunk metadata
- Always include section field in output (for downstream processing)
- Handle edge cases: very long sentences, empty paragraphs, None sections
...
### 8.2 Delta Addition: Parent/Child Chunking Module

> **Source:** RAG_IMPROVEMENTS_DELTA.md - Priority 1: Parent/Child Chunking
>
> This addition enables sophisticated news impact analysis by storing documents at two granularities:
...
- [backend.mdc] for Python patterns
- See RAG_IMPROVEMENTS_DELTA.md Edge Cases section for missing metadata handling
...
# Simulate a child chunk structure
child_chunk = {'text': chunks[0], 'chunk_index': 0, 'start_page': 1, 'section': 'Item 1A: Risk Factors'}
enriched = enricher.enrich_chunk(
    child_chunk,
    {'document_id': 'AAPL_10K_2024', 'document_type': '10k', 'company': 'Apple Inc.', 'ticker': 'AAPL', 'fiscal_year': 2024, 'total_pages': 127}
)
print(f'Enriched text preview: {enriched[\"text\"][:250]}...')
print(f'Raw text (for citation): {enriched[\"text_raw\"][:100]}...')
"
```
...
- [ ] Chunking respects section boundaries (10-K documents)
- [ ] Contextual enrichment prepends type-specific metadata prefix
- [ ] Enriched chunks have both `text` (for embedding) and `text_raw` (for citation)
- [ ] Enrichment applied to child chunks only (parents stored as-is)
...
> **Delta Addition:** Parent/Child Chunking Support
> Child chunks are embedded and stored; parent text is included in metadata for retrieval.
> See RAG_IMPROVEMENTS_DELTA.md for full architecture details.

{
  "id": "AAPL_10K_2024_parent_5_child_2",  # Child chunk ID
  "values": [0.1, 0.2, ...],  # 1536 floats (child embedding)
  "metadata": {
    "document_id": "AAPL_10K_2024",
    "document_type": "10k",
    ...
    # Parent/Child Fields (Delta Addition)
...
| total_pages | ✓ | ✓ | Total document pages for context |
| **parent_id** | ✓ | ✓ | **Delta:** Links child to parent chunk |
| **parent_index** | ✓ | ✓ | **Delta:** Parent position in document |
| **child_index** | ✓ | ✓ | **Delta:** Child position within parent |
| **parent_text** | ✓ | ✓ | **Delta:** Full 1024-token context for LLM |
| **child_text** | ✓ | ✓ | **Delta:** Enriched 256-token text (embedded) |
| **child_text_raw** | ✓ | ✓ | **Delta:** Raw child text for citations |

**Delta Addition: Query Returns parent_text**

The query method should return `parent_text` for LLM context while keeping `child_text_raw` for citations:

```python
def query(self, vector: list[float], top_k: int = 10, filter: dict = None) -> list[dict]:
    """Query returns parent_text for LLM, child_text for citation."""
    results = self._index.query(vector=vector, top_k=top_k, filter=filter, include_metadata=True)
    # Results include parent_text (1024 tokens) for full context
    # Results include child_text_raw for precise citation
    return results

# Metadata size consideration:
# - Pinecone metadata limit: 40KB per vector
# - 1024 tokens ≈ 4KB text (well under limit)
# - Include both parent_text and child_text safely
```
...
Changes:
1. After extraction, process JSON through parent/child chunking pipeline
2. Generate embeddings for child chunks only
3. Upsert to Pinecone with parent_text in metadata
...
- needs_reindexing(manifest: dict, doc_id: str) -> bool

> **Delta Addition: Parent/Child Indexing Pipeline**
>
> The indexing pipeline now uses parent/child chunking for precise retrieval with full context.
> Child chunks (256 tokens) are embedded; parent text (1024 tokens) stored in metadata.
> See RAG_IMPROVEMENTS_DELTA.md for architecture details.

Indexing Pipeline Per Document:
1. Load extracted JSON
2. Check manifest - skip if already indexed AND schema version matches (unless --reindex)
3. **Create parent/child chunks using ParentChildChunker** (returns parents, children)
4. **Enrich CHILDREN ONLY using ContextualEnricher** (parents stored as-is)
5. **Generate embeddings for children using BedrockEmbeddings**
6. **Build Pinecone vectors with parent_text in metadata:**
   ```python
   for child in enriched_children:
       ...
       vector = {
           ...
           "metadata": {
               ...
               "child_text_raw": child["text_raw"],  # For citations
               "section": child["section"],
               "page_number": child["start_page"],
               ...
               # Reference doc fields (if applicable)
   ...
   ```
7. **Delete existing vectors for document_id** (prevents duplicates)
8. Upsert to Pinecone with metadata
9. **Update manifest** with index_schema_version:
   ```python
   ...
   manifest["documents"][doc_id].update({
       ...
       "chunk_count": len(children),  # Child chunks (vectors)
       "parent_count": len(parents),  # Parent chunks
   })
   ```
10. Log progress and stats

CLI Updates:
- --index-only: Skip extraction, just index existing JSON (checks manifest for unindexed docs)
- --reindex: Delete existing vectors and re-index all documents (ignores schema version)
...
"  Created 65 parents, 260 children"
"  Generated 260 embeddings"
"  Upserting to Pinecone..."
"  Done: 260 vectors indexed (65 unique parents)"

Summary Output:
"Indexing Summary:
  Documents indexed: 11
  Total parents: 715
  Total children (vectors): 2,860
  Pinecone index stats: 2,860 vectors in 1 namespace"

Reference:
- ParentChildChunker from Section 8.2 Delta Addition
- ContextualEnricher from Section 8.3
- BedrockEmbeddings from Section 8.1
...
- retrieve_documents(query: str, top_k: int = 5, filters: dict = None) -> str:
  ...
  - Step 4: **Delta Addition:** Deduplicate results by parent_id
  - Step 5: Format results with citations using parent_text
  - Return formatted response

> **Delta Addition: Parent/Child Retrieval**
> 
> Multiple child chunks may match from the same parent. Deduplicate by parent_id 
> and return the full parent_text (1024 tokens) for LLM context.
> See RAG_IMPROVEMENTS_DELTA.md for full architecture details.

**Delta Addition: Deduplicate by Parent Algorithm**
...
Response Format (Delta Addition: uses parent_text):
...
- **Delta Addition:** Deduplicate by parent_id to avoid duplicate passages
- **Delta Addition:** Return parent_text for full context (1024 tokens)
- **Delta Addition:** Include child_text_raw match preview in citation

**Delta Addition: Backwards Compatibility**

Handle case where parent_text is missing (old schema vectors):

```python
def get_result_text(metadata: dict) -> str:
    """Get text content with backwards compatibility."""
    ...
    if not metadata.get("parent_text"):
        logger.warning("missing_parent_text", document_id=metadata.get("document_id"))
...
- [ ] **Delta:** Deduplicate results by parent_id
- [ ] **Delta:** Return parent_text (1024 tokens) for full context
- [ ] **Delta:** Include child_text_raw match preview in citations
- [ ] **Delta:** Backwards compatibility for missing parent_text
